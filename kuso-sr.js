		...
		if(choice == ''){
			...
			prompt.question("\n >>> ",(ttl) => {
				if(ttl == ''){
					...
					HttpPage(
						`https://www.kusonime.com/?s=${ttl.replace(/\s+/g,'+')}`, function(result) {
							findTitle(result);
						});
				}
			});
...
function findTitle(page){
	...
	$("div[class='navigation']").find("a").each((x,y) => {
		const i = $(y);
		if (i.text()){
	...
	$("h2[class='episodeye']").find("a").each((i,elem) => {
		...
		prompt.question("\n >>> ",(il) => {
			if(il !== ''){
				if((il-1) < info.length){
					HttpPage(
						info[il-1], function(e){
							parseInformation(e)
						});
				} else if(il === "N"){
					if (next.length !== 0){
						HttpPage(
							next[0], function(result) {
								findTitle(result);
							});
					} else {
...
function parseInformation(page){
	clear();console.log(banner);console.log(`\t\t\t${blue}[${white}·Anime·Information·${blue}]\n${white}`);
	const $ = cheerio.load(page);
	console.log(` ${blue}[${white}^${blue}]${white} Title : ${$("h1.jdlz").text()}`);
	var up = $("div.kategoz").text().split(/\s\s\s/);
	console.log(` ${blue}[${white}^${blue}]${white} Uploads : ${up[0]}
 ${blue}[${white}^${blue}]${white} Viewers : ${up[1]}`)
	$("div.info").find("p").each(function(i,elem){
		console.log(` ${blue}[${white}^${blue}]${white} ${$(elem).text()}`);
	});
	var des = page.match(/<\/strong>(.+?)<strong>/i)[1].replace(/<\/\w.*|<\w.*/,'').trim();
...
function DownloadPage(page){
	...
	const link = [];
	const no_server = [];
	clear();console.log(banner);console.log(`\t\t\t ${blue}[${white} Download Page ${blue}]${white}\n`);
	const $ = cheerio.load(page);
	$("div[class='smokeddl']").each((i,res) => {
		smokeddl.push($(res).html());
		const smko = $(res)
			.find("div[class='smokettl']")
			.text();
		if(smko.length !== 0){
			smokettl.push(smko);
		}
						...
						smoke("div[class='smokeurl']").each((i,elem) => {
							...
							no_server.push(el.html());
								...
								console.log(` ${i+1}${blue}).${white} ${e}`);
									...
									console.log(`\t\t\t ${blue}[${white} List Server ${blue}]${white} \n`);
									if(rs !== ''){
										if((rs-1) < reso.length){
											const no = cheerio.load(no_server[rs-1]);
												...
												link.push(sv
													...
													console.log(` ${white}${i+1}${blue}). ${white}${y} ${blue}>>${white} ${link[i]}`);
												});
										} else {
